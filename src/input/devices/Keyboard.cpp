...
const std::map<TriggerType, std::function<bool(int)>> Keyboard::RAYLIB_MAPPINGS{
    {TRIGGER_PRESSED, IsKeyPressed},
...
std::list<GameEvent> Keyboard::getGameEvents() {
    ...
    for (auto it = INPUT_MAPPINGS.cbegin(); it != INPUT_MAPPINGS.end(); ++it) {
        ...
        if (Keyboard::RAYLIB_MAPPINGS.at(deviceEvent.triggerType)(
                deviceEvent.trigger)) {
